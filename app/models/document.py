...
class Document(SQLModel, table=True):
    ...
    current_version_id: Optional[UUID] = Field(default=None, foreign_key="document_versions.id")
    ...
    versions: List["DocumentVersion"] = Relationship(
        ...
        sa_relationship_kwargs={"cascade": "all, delete-orphan", "primaryjoin": "Document.id==DocumentVersion.document_id"}
    )
    chunks: List["DocumentChunk"] = Relationship(back_populates="document", sa_relationship_kwargs={"cascade": "all, delete-orphan"})
    jobs: List["ProcessingJob"] = Relationship(back_populates="document", sa_relationship_kwargs={"cascade": "all, delete-orphan"})
...
class DocumentVersion(SQLModel, table=True):
    ...
    parent_version_id: Optional[UUID] = Field(default=None, foreign_key="document_versions.id")
    ...
    document: Document = Relationship(
        ...
        sa_relationship_kwargs={"primaryjoin": "DocumentVersion.document_id==Document.id"}
    )
